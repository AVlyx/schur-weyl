...
def partitions(k: int, max_part: Optional[int] = None, max_height: Optional[int] = None) -> Generator[list[int]]:
    ...
    Yields:
        Generator[list[int]]: The partitions.
...
def cells(lam: list[int]) -> Generator[tuple[int, int]]:
...
def majorizes(lam: list[int], nu: list[int]) -> bool:
    ...
    Args:
        lam (list[int]): a partition
        nu (list[int]): a partition of the same size as lambda. Otherwise the result is undefined
...
def addable_corners(lam: list[int]) -> Generator[tuple[int, int]]:
...
def removable_corners(lam: list[int]):
